...
@celery_app.task(name="metaprofile.profile_project.workers.incremental_builder.run")
def run() -> dict:
    """从昨日清洗后的中间表抓取新数据，跑 NER + 抽取 + 入库。

    流程：
    1. 查询 cleaned_data WHERE source_date = yesterday AND processed = false
    2. 按批次（默认 100 条/批）调用 foundation.ner + foundation.extractors.project_extractor
