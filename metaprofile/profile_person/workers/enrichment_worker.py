...
@celery_app.task(
    name="metaprofile.profile_person.workers.enrichment_worker.scan_and_enrich"
)
def scan_and_enrich() -> dict:
    """扫描完整度 < 60% 的画像，批量 RAG 补全。

    流程：
    1. SELECT person_id FROM person_profile WHERE completeness < 0.60
...
@celery_app.task(
    name="metaprofile.profile_person.workers.enrichment_worker.enrich_one"
)
def enrich_one(person_id: str) -> dict:
    """单个画像的按需补全（被 enrichment API 调用）。"""
    logger.info("tech_single_enrich", person_id=person_id)
