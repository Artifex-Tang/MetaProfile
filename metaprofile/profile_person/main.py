...
启动命令：
    uvicorn metaprofile.profile_person.main:app --host 0.0.0.0 --port 8004

对应课题任务书指标 4.2 的"产业前沿科技人员画像"工具。
"""
...
async def lifespan(_app: FastAPI):
    logger.info("profile_person_starting", env=settings.env)
    yield
    logger.info("profile_person_stopping")


app = FastAPI(
    title="产业前沿科技人员画像服务",
    description=(
        "提供人员实体画像的查询/搜索/语义检索/批量查询/更新/导入/统计/"
        "关系查询/路径查询/变更查询/RAG补全 共 11 个标准 REST 接口。"
        "字段定义严格遵循《实体画像数据规范》人员节。"
    ),
    version="1.0.0",
    lifespan=lifespan,
)
...
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "profile_person"}
