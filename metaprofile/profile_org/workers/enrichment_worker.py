...
from metaprofile.profile_org.workers.celery_app import celery_app

logger = structlog.get_logger(__name__)


@celery_app.task(
    name="metaprofile.profile_org.workers.enrichment_worker.scan_and_enrich"
)
def scan_and_enrich() -> dict:
    """扫描完整度 < 60% 的画像，批量 RAG 补全。

    流程：
    1. SELECT org_id FROM org_profile WHERE completeness < 0.60
...
@celery_app.task(
    name="metaprofile.profile_org.workers.enrichment_worker.enrich_one"
)
def enrich_one(org_id: str) -> dict:
    """单个画像的按需补全（被 enrichment API 调用）。"""
    logger.info("tech_single_enrich", org_id=org_id)
