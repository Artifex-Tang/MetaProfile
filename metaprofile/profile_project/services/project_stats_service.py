...
from metaprofile.profile_project.domain.orm_models import EntityChangeLogORM, ProjectProfileORM
...
_CACHE_KEY_ID = "project:latest"
_CACHE_TTL = 3600
_PERIOD_DAYS = 30


class ProjectStatsService:
    """计算项目画像综合统计：总量、本期增量、领域分布、完整度分布、LLM 贡献度。
    ...
    async def compute(self, session: AsyncSession) -> ProjectStatsResponse:
    ...
    async def _compute_live(self, session: AsyncSession) -> ProjectStatsResponse:
        total: int = (
            await session.execute(
                select(func.count()).select_from(ProjectProfileORM)
            )
        ).scalar_one()
        ...
        total_changes: int = (
            await session.execute(
                select(func.count())
                .select_from(EntityChangeLogORM)
                .where(EntityChangeLogORM.entity_type == "project")
            )
        ).scalar_one()
        ...
        hist_rows = (
            await session.execute(
                text(
                    ...
                    "COUNT(*) AS cnt FROM project_profile GROUP BY bucket"
                )
            )
        ).all()
