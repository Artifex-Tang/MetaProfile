...
from metaprofile.profile_project.workers.celery_app import celery_app

logger = structlog.get_logger(__name__)


@celery_app.task(
    name="metaprofile.profile_project.workers.enrichment_worker.scan_and_enrich"
)
def scan_and_enrich() -> dict:
    """扫描完整度 < 60% 的画像，批量 RAG 补全。

    流程：
    1. SELECT project_id FROM project_profile WHERE completeness < 0.60
...
@celery_app.task(
    name="metaprofile.profile_project.workers.enrichment_worker.enrich_one"
)
def enrich_one(project_id: str) -> dict:
    """单个画像的按需补全（被 enrichment API 调用）。"""
    logger.info("tech_single_enrich", project_id=project_id)
