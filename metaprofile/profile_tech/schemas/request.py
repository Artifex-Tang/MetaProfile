...
class SearchRequest(_Req):
    ...
    tech_domain: list[str] | None = None
...
class SemanticSearchRequest(_Req):
    ...
    tech_domain_filter: list[str] | None = None
...
class UpdateTechProfileRequest(_Req):
    """字段级更新：所有字段可选，仅更新非 None 字段。"""

    tech_name_cn: str | None = None
...
class BulkImportRequest(_Req):
    ...
    profiles: list[TechProfile] = Field(..., min_length=1, max_length=1000)
