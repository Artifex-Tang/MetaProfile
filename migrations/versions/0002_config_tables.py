...
def upgrade() -> None:
    # 0001 用 Base.metadata.create_all 已建全部 ORM 表（含本批配置表），
    # 这里用 has_table 守卫保证幂等，避免空库升级时 DuplicateTable。
    from sqlalchemy import inspect
    insp = inspect(op.get_bind())

    if not insp.has_table("llm_provider_configs"):
        op.create_table(
    ...
    if not insp.has_table("data_source_configs"):
        op.create_table(
            "data_source_configs",
            sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
            sa.Column("name", sa.String(128), nullable=False),
    ...
    if not insp.has_table("collection_tasks"):
        op.create_table(
            "collection_tasks",
            sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
        ...
        if not insp.has_index("collection_tasks", "ix_collection_tasks_source_id"):
            op.create_index("ix_collection_tasks_source_id", "collection_tasks", ["source_id"])
        if not insp.has_index("collection_tasks", "ix_collection_tasks_status"):
